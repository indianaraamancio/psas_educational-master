...
export class TeamComponent implements OnInit {
  ...
  getTeams() {
    ...
    this.service.findTeamsFromUser().subscribe((data) => {
      ...
      if (p.organizationId === this.organizationId) {
        console.log("Id da org: "+p.organizationId);
  ...
  addAllMembers(){
    this.membersNotInTeam.forEach(m =>{
      let emailMember = m.email;
      this.addMemberToTeam(emailMember);
    });
    this.initializeComponents();   
  }
